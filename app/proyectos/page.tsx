      ...
      <main className="flex-1 p-6 md:p-8">
        <div className="mx-auto max-w-7xl space-y-8">
          ...
          <nav className="flex flex-wrap gap-2">{([
            ["resumen", "Resumen"], ["proyectos", "Proyectos / centros de costo"], ["presupuestos", "Presupuestos"], ["movimientos", "Movimientos"], ["proximamente", "Proximamente"],
          ...
          {tab === "proximamente" && <Panel titulo="Proximamente" subtitulo="Integraciones deshabilitadas hasta completar fases posteriores."><div className="flex flex-wrap gap-3">{["Vincular con cheque", "Vincular con planilla", "Vincular con CxP/CxC", "Generar asiento contable", "Reporte por proyecto", "Cerrar proyecto con contabilidad", "Importar presupuesto"].map((label) => <button key={label} type="button" disabled className="inline-flex items-center gap-2 rounded-xl border border-[var(--card-border)] bg-[var(--surface)] px-4 py-3 text-sm font-bold text-[var(--muted)] opacity-70"><Lock size={14} />{label} - Proximamente</button>)}</div></Panel>}
        </div>
      </main>
    </div>
  );

...
import { ClipboardList, FileText, Lock, ShieldCheck, Users } from "lucide-react";
import Sidebar from "../../components/Sidebar";
import { validarAccesoModuloUsuario } from "../../lib/validarAccesoModuloUsuario";

const resumen = [
  { titulo: "Empleados", valor: "Pendiente de conectar" },
  { titulo: "IGSS / IRTRA / INTECAP", valor: "Sin conexión todavía" },
  { titulo: "Boletas", valor: "Pendiente de conectar" },
  { titulo: "Asiento revisable", valor: "Preparado para fase SQL" },
];
...
const funciones = [
  ...
  "Boletas",
  "CxP de planilla",
  "Asiento contable revisable",
];

const proximosPasos = [
  "Conectar catálogo de empleados",
  "Preparar cálculos de prestaciones e impuestos laborales",
  "Vincular boletas y CxP de planilla",
  "Enviar asiento a revisión contable en fase posterior",
];
    ...
    async function iniciar() {
      try {
        ...
        if (!acceso.ok) {
          ...
          setMensajeBloqueo("No tienes acceso al módulo Planilla.");
          setAutorizado(false);
        ...
        console.error("Error validando acceso a planilla:", error);
        if (activo) {
          setMensajeBloqueo("No se pudo validar el acceso al módulo Planilla.");
          setAutorizado(false);
          setValidandoAcceso(false);
        }
      }
          ...
          <header className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
              <div className="flex flex-wrap items-center gap-3">
                <Users className="text-cyan-500" size={42} />
                <h1 className="text-4xl md:text-5xl font-black">Planilla / IGSS / IRTRA / INTECAP</h1>
              ...
              <p className="mt-3 max-w-3xl text-[var(--muted)]">
                Vista base para planilla, obligaciones laborales y asiento contable revisable.
              </p>
            </div>
    ...
    <section className="rounded-2xl border border-cyan-400/30 bg-cyan-400/10 p-5 text-cyan-100">
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="mt-1">Este módulo está preparado para conexión futura con Supabase, auditoría, documentos y reportes. Por ahora no guarda datos.</p>
        </div>
      </div>
    </section>
  );

-- Preflight search_path RPC Supabase V1
...
SELECT
  ...
  COALESCE(
    ...
    'SIN_EXPLICITO'
  ) AS search_path_actual,
  CASE
    WHEN p.oid IS NULL THEN false
    ELSE EXISTS (
      SELECT 1
      FROM unnest(p.proconfig) AS configuracion
      WHERE configuracion = 'search_path=pg_catalog, public, pg_temp'
    )
  END AS cumple_search_path_seguro
FROM objetivo o
LEFT JOIN pg_proc p
  ON p.oid = to_regprocedure(o.firma_esperada)
ORDER BY o.firma_esperada;
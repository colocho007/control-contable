-- Propuesta de remediaciÃ³n de permisos RPC Supabase V1
-- Generada desde la matriz documental de 41 funciones.
-- NO EJECUTAR EN REMOTO SIN REVISIÃ“N Y APROBACIÃ“N.
-- No modifica cuerpos, propietarios ni search_path.

BEGIN;

-- VerificaciÃ³n previa: todas las firmas deben existir.
DO $remediation$
BEGIN
  IF to_regprocedure('public.contabilidad_autorizado(bigint, text[], text[])') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.contabilidad_autorizado(bigint, text[], text[])';
  END IF;
  IF to_regprocedure('public.contabilidad_empresa_permitida(bigint)') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.contabilidad_empresa_permitida(bigint)';
  END IF;
  IF to_regprocedure('public.monitoreo_alertas_set_actualizado_at()') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.monitoreo_alertas_set_actualizado_at()';
  END IF;
  IF to_regprocedure('public.movimientos_empresa_asignada(bigint)') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.movimientos_empresa_asignada(bigint)';
  END IF;
  IF to_regprocedure('public.movimientos_puede_anular(bigint)') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.movimientos_puede_anular(bigint)';
  END IF;
  IF to_regprocedure('public.movimientos_puede_escribir(bigint)') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.movimientos_puede_escribir(bigint)';
  END IF;
  IF to_regprocedure('public.validar_anulacion_movimiento_operativo()') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.validar_anulacion_movimiento_operativo()';
  END IF;
  IF to_regprocedure('public.anular_asiento_contable(uuid, bigint, text, uuid, text)') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.anular_asiento_contable(uuid, bigint, text, uuid, text)';
  END IF;
  IF to_regprocedure('public.registrar_asiento_completo(bigint, uuid, date, text, text, text, jsonb, uuid, text)') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.registrar_asiento_completo(bigint, uuid, date, text, text, text, jsonb, uuid, text)';
  END IF;
  IF to_regprocedure('public.cerrar_periodo_contable(uuid, bigint, text, uuid, text)') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.cerrar_periodo_contable(uuid, bigint, text, uuid, text)';
  END IF;
  IF to_regprocedure('public.anular_cheque_transaccional(bigint, bigint, uuid, text, text)') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.anular_cheque_transaccional(bigint, bigint, uuid, text, text)';
  END IF;
  IF to_regprocedure('public.autorizar_cheque_transaccional(bigint, bigint, uuid, text)') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.autorizar_cheque_transaccional(bigint, bigint, uuid, text)';
  END IF;
  IF to_regprocedure('public.crear_cheque_transaccional(bigint, bigint, date, text, text, numeric, text, numeric, text, text, text, uuid, bigint, bigint, text, text, timestamp with time zone, uuid, text)') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.crear_cheque_transaccional(bigint, bigint, date, text, text, numeric, text, numeric, text, text, text, uuid, bigint, bigint, text, text, timestamp with time zone, uuid, text)';
  END IF;
  IF to_regprocedure('public.pagar_cheque_transaccional(bigint, bigint, uuid, text)') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.pagar_cheque_transaccional(bigint, bigint, uuid, text)';
  END IF;
  IF to_regprocedure('public.rechazar_cheque_transaccional(bigint, bigint, uuid, text, text)') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.rechazar_cheque_transaccional(bigint, bigint, uuid, text, text)';
  END IF;
  IF to_regprocedure('public.contabilizar_documento_contable(uuid, bigint, uuid, text)') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.contabilizar_documento_contable(uuid, bigint, uuid, text)';
  END IF;
  IF to_regprocedure('public.finalizar_asiento_contable(uuid, bigint, uuid, text)') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.finalizar_asiento_contable(uuid, bigint, uuid, text)';
  END IF;
  IF to_regprocedure('public.eliminar_empresa_vacia_segura(bigint, text)') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.eliminar_empresa_vacia_segura(bigint, text)';
  END IF;
  IF to_regprocedure('public.anular_pago_cxc(text, bigint, uuid, text, text)') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.anular_pago_cxc(text, bigint, uuid, text, text)';
  END IF;
  IF to_regprocedure('public.anular_pago_cxp(text, bigint, uuid, text, text)') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.anular_pago_cxp(text, bigint, uuid, text, text)';
  END IF;
  IF to_regprocedure('public.registrar_pago_cxc(text, bigint, date, text, text, text, text, numeric, text, uuid, text)') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.registrar_pago_cxc(text, bigint, date, text, text, text, text, numeric, text, uuid, text)';
  END IF;
  IF to_regprocedure('public.registrar_pago_cxp(text, bigint, date, text, text, text, text, numeric, text, uuid, text)') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.registrar_pago_cxp(text, bigint, date, text, text, text, text, numeric, text, uuid, text)';
  END IF;
  IF to_regprocedure('public.registrar_rate_limit_operativo(text, text, text, text, integer, integer, bigint, text, jsonb)') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.registrar_rate_limit_operativo(text, text, text, text, integer, integer, bigint, text, jsonb)';
  END IF;
  IF to_regprocedure('public.seguridad_operativa_set_actualizado_at()') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.seguridad_operativa_set_actualizado_at()';
  END IF;
  IF to_regprocedure('public.actualizar_empleado_v2(uuid, integer, jsonb, text)') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.actualizar_empleado_v2(uuid, integer, jsonb, text)';
  END IF;
  IF to_regprocedure('public.crear_empleado_v2(jsonb, text)') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.crear_empleado_v2(jsonb, text)';
  END IF;
  IF to_regprocedure('public.empleados_empresa_permitida_v2(bigint)') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.empleados_empresa_permitida_v2(bigint)';
  END IF;
  IF to_regprocedure('public.empleados_fallar_operacion_v2(uuid, text)') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.empleados_fallar_operacion_v2(uuid, text)';
  END IF;
  IF to_regprocedure('public.empleados_puede_escribir_v2(bigint)') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.empleados_puede_escribir_v2(bigint)';
  END IF;
  IF to_regprocedure('public.empleados_puede_estado_v2(bigint)') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.empleados_puede_estado_v2(bigint)';
  END IF;
  IF to_regprocedure('public.empleados_puede_sensible_v2(bigint)') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.empleados_puede_sensible_v2(bigint)';
  END IF;
  IF to_regprocedure('public.empleados_reservar_operacion_v2(text, text, text, text)') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.empleados_reservar_operacion_v2(text, text, text, text)';
  END IF;
  IF to_regprocedure('public.empleados_snapshot_auditable_v2(public.empleados_planilla)') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.empleados_snapshot_auditable_v2(public.empleados_planilla)';
  END IF;
  IF to_regprocedure('public.empleados_try_bigint_v2(text)') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.empleados_try_bigint_v2(text)';
  END IF;
  IF to_regprocedure('public.empleados_try_date_v2(text)') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.empleados_try_date_v2(text)';
  END IF;
  IF to_regprocedure('public.empleados_try_integer_v2(text)') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.empleados_try_integer_v2(text)';
  END IF;
  IF to_regprocedure('public.empleados_try_numeric_v2(text)') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.empleados_try_numeric_v2(text)';
  END IF;
  IF to_regprocedure('public.empleados_try_uuid_v2(text)') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.empleados_try_uuid_v2(text)';
  END IF;
  IF to_regprocedure('public.empleados_validar_fila_v2(jsonb)') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.empleados_validar_fila_v2(jsonb)';
  END IF;
  IF to_regprocedure('public.importar_empleados_v2(text, text, bigint, text, text, jsonb)') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.importar_empleados_v2(text, text, bigint, text, text, jsonb)';
  END IF;
  IF to_regprocedure('public.validar_importacion_empleados_v2(text, text, jsonb)') IS NULL THEN
    RAISE EXCEPTION 'FunciÃ³n no encontrada: public.validar_importacion_empleados_v2(text, text, jsonb)';
  END IF;
END;
$remediation$;

-- CategorÃ­a: AUTENTICADA
...
-- CategorÃ­a: CANDIDATA_REVOCACION
...
-- CategorÃ­a: INTERNA_SERVICE_ROLE
...
-- Fin de la propuesta. ProducciÃ³n continÃºa NO-GO.
